...
export default function SendMoneyForm() {
  ...
  const handleSubmit = (e) => {
    ...
    if (
      input.sender &&
      !(
        (input.sender.startsWith("01") ||
          input.sender.startsWith("+8801") ||
          input.sender.startsWith("8801")) &&
        (input.sender.length === 11 ||
          input.sender.length === 14 ||
          input.sender.length === 13)
      )
    ) {
      toast.error(
    ...
    if (
      input.receiver &&
      !(
        (input.receiver.startsWith("01") ||
          input.receiver.startsWith("+8801") ||
          input.receiver.startsWith("8801")) &&
        (input.receiver.length === 11 ||
          input.receiver.length === 14 ||
          input.receiver.length === 13)
      )
    ) {
    ...
    if (success) {
      if (
        input.sender.replace("+8801", "") ===
          input.receiver.replace("+8801", "") ||
        input.sender.replace("8801", "") ===
          input.receiver.replace("8801", "") ||
        input.sender.replace("01") === input.receiver.replace("01", "")
      ) {
      ...
      toast.error(
        "আপনি আপনার বন্ধুর সাথে মজা নিতে এসে। এই অ্যাপ্লিকেশনের সাথেই মজা নিতে চাচ্ছেন!😆"
      );
      return;
    }
  };
  ...
  return (
    <>
      {showModal && (
        <Modal money={input} onModal={setShowModal} reset={setInput} />
      )}
